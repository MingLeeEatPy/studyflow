...
- 在“专注设置 → 跨设备同步”注册或登录后，应用会自动上传或恢复云端数据。
- 另一台设备使用同一 Google 或邮箱密码账号登录后会自动同步；只有两边都已有独立学习数据时才询问如何合并。
- StudyFlow 不提供手机号绑定或短信验证码功能。
...
IndexedDB 是离线缓存；登录后 Supabase 是云端主数据。同步采用本地 outbox、增量游标和按更新时间索引，服务器压力主要来自云端记录、同步请求、认证邮件、数据库连接和磁盘，而不是用户本地缓存本身。用户增长后优先升级数据库计算资源、连接池和监控，再增加队列、归档与对象存储。
