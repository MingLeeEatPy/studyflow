import { ArrowLeft, Minimize2, Pause, Play, SkipForward, Square } from "lucide-react";
...
export function MeditationFocusPage({ session, elapsedSeconds, phaseElapsedSeconds, growthStage, growthVariant, overtime, onLeave, onCompact, onSkipBreathing, onPause, onResume, onFinish }: { session: MeditationSession; elapsedSeconds: number; phaseElapsedSeconds: number; growthStage: GrowthStage; growthVariant: number; overtime: boolean; onLeave: () => void; onCompact: () => void; onSkipBreathing: () => void; onPause: () => void; onResume: () => void; onFinish: () => void }) {
  ...
  return <main className="meditation-focus-page">
    ...
    <header><button aria-label="返回 StudyFlow" onClick={onLeave}><ArrowLeft/></button><span>StudyFlow · Meditation</span><div className="meditation-header-actions"><small>{intentionLabels[session.intention]}</small><button aria-label="缩小计时器" onClick={onCompact}><Minimize2/></button></div></header>
