...
export function PlantIllustration({ kind, stage, variant = 0, overtime = false }: {
  ...
  if (kind === "flower") {
    return <svg className={`plant-illustration flower-stage stage-${stage} ${hueClass}${overtime ? " is-overtime" : ""}`} viewBox="0 0 280 300" role="img" aria-label={["花种", "花芽", "幼苗", "花苞", "盛开的花"][stage]}>
      <defs>
        <linearGradient id={`flower-stem-${uniqueId}`} x1="0" y1="0" x2="1" y2="1"><stop stopColor="#9ab9a8"/><stop offset="1" stopColor="#527665"/></linearGradient>
        <radialGradient id={`petal-${uniqueId}`} cx="45%" cy="35%"><stop stopColor="#f4eee0"/><stop offset="1" stopColor="#c7b88f"/></radialGradient>
        <filter id={`flower-glow-${uniqueId}`}><feGaussianBlur stdDeviation="8"/></filter>
      </defs>
      ...
      {stage >= 1 && <path
        ...
        d={`M140 253 Q${stage > 2 ? 128 : 145} ${225 - stage * 27} 140 ${205 - stage * 28}`}
        stroke={`url(#flower-stem-${uniqueId})`}
      />}
      ...
      {stage >= 3 && <g className="flower-head" transform={`translate(140 ${116 - (stage - 3) * 23})`}>
        {stage === 3 ? <path className="bud" style={{ fill: `url(#petal-${uniqueId})` }} d="M0 20c-24-14-18-46 0-55 18 9 24 41 0 55Z"/> : <>
          <circle className="plant-glow" r="67" fill="#d7b77a" filter={`url(#flower-glow-${uniqueId})`}/>
          {[0,60,120,180,240,300].map(angle => <ellipse key={angle} className="petal" style={{ fill: `url(#petal-${uniqueId})` }} rx="19" ry="43" cy="-31" transform={`rotate(${angle})`}/>) }
  ...
  return <svg className={`plant-illustration tree-stage stage-${stage} ${hueClass}${overtime ? " is-overtime" : ""}`} viewBox="0 0 320 320" role="img" aria-label={["种子", "嫩芽", "幼苗", "小树", "成熟树"][stage]}>
    <defs>
      <linearGradient id={`trunk-${uniqueId}`} x1="0" y1="0" x2="1" y2="1"><stop stopColor="#9b8061"/><stop offset="1" stopColor="#4f4637"/></linearGradient>
      <linearGradient id={`leaf-${uniqueId}`} x1="0" y1="0" x2="1" y2="1"><stop stopColor="#9ab9a8"/><stop offset="1" stopColor="#456c5a"/></linearGradient>
      <filter id={`tree-glow-${uniqueId}`}><feGaussianBlur stdDeviation="10"/></filter>
    </defs>
    ...
    {stage >= 1 && <path className="tree-trunk" fill="none" stroke={`url(#trunk-${uniqueId})`} strokeWidth={stage >= 4 ? 15 : stage >= 3 ? 11 : 7} d={`M160 274 Q${stage >= 3 ? 150 : 164} ${233 - stage * 16} 160 ${210 - stage * 31}`} />}
    ...
    {stage >= 1 && <g className="tree-leaves" style={{ fill: `url(#leaf-${uniqueId})` }}>
    ...
    {overtime && stage === 4 && <><circle className="plant-glow" cx="160" cy="130" r="97" fill="#d7b77a" filter={`url(#tree-glow-${uniqueId})`}/><g className="overtime-leaves"><ellipse cx="83" cy="102" rx="6" ry="12"/><ellipse cx="239" cy="119" rx="6" ry="12"/><ellipse cx="221" cy="66" rx="5" ry="10"/></g></>}
  </svg>;
}
